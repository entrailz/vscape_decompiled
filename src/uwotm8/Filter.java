...
public final class Filter
{
    ...
    public final int a(final int n, final float n2) {
        if (n == 0) {
            Filter.c = (int)((Filter.h = (float)Math.pow(0.1, (this.f[0] + (this.f[1] - this.f[0]) * n2) * 0.003051758f / 20.0f)) * 65536.0f);
        }
        if (this.a[n] == 0) {
            return 0;
        }
        final float a = this.a(n, 0, n2);
        Filter.g[n][0] = -2.0f * a * (float)Math.cos(this.a(n2, 0, n));
        final float[] array = Filter.g[n];
        final int n3 = 1;
        final float n4 = a;
        array[n3] = n4 * n4;
        for (int i = 1; i < this.a[n]; ++i) {
            ...
            Filter.g[n][(i << 1) + 1] = Filter.g[n][(i << 1) - 1] * n7;
            Filter.g[n][i << 1] = Filter.g[n][(i << 1) - 1] * n5 + Filter.g[n][(i << 1) - 2] * n7;
            for (int j = (i << 1) - 1; j >= 2; --j) {
                final float[] array2 = Filter.g[n];
                final int n8 = j;
                array2[n8] += Filter.g[n][j - 1] * n5 + Filter.g[n][j - 2] * n7;
            }
            final float[] array3 = Filter.g[n];
            final int n9 = 1;
            array3[n9] += Filter.g[n][0] * n5 + n7;
            final float[] array4 = Filter.g[n];
            final int n10 = 0;
            array4[n10] += n5;
        }
        if (n == 0) {
            for (int k = 0; k < this.a[0] << 1; ++k) {
                final float[] array5 = Filter.g[0];
                final int n11 = k;
                array5[n11] *= Filter.h;
            }
        }
        for (int l = 0; l < this.a[n] << 1; ++l) {
            Filter.b[n][l] = (int)(Filter.g[n][l] * 65536.0f);
        }
        return this.a[n] << 1;
    }
    
    public final void a(final Buffer i, final Envelope am) {
        final int e = i.e();
        this.a[0] = e >> 4;
        this.a[1] = (e & 0xF);
        if (e != 0) {
